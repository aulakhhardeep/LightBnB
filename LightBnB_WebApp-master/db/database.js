...
const getAllProperties = function (options, limit = 10) {
  ...
  if (options.city) {
    ...
    } properties.city LIKE $${queryParams.length} `;
  }
  ...
  if (options.minimum_price_per_night && options.maximum_price_per_night) {
    queryParams.push(
      Number(options.minimum_price_per_night) * 100,
      Number(options.maximum_price_per_night) * 100
    );
    queryString += `${
      queryParams.length > 2 ? "AND" : "WHERE"
    } properties.cost_per_night BETWEEN $${queryParams.length - 1} AND $${
      queryParams.length
    } `;
  }
